...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            log(f"⚠️ Config error: {e}. Using defaults.")
    return config
...
def resolve_input_device(config):
    ...
    try:
        ...
        log(f"[AUDIO] Device query failed: {e}")
        return None
    ...
    if isinstance(requested, int) or (isinstance(requested, str) and requested.isdigit()):
        ...
        if 0 <= index < len(devices):
            return index
        log(f"[AUDIO] Input device index not found: {index}")
        return None

    requested_lower = str(requested).lower()
    for idx, dev in enumerate(devices):
        log(f"[AUDIO DEBUG] Index {idx}: {dev.get('name')} (In: {dev.get('max_input_channels')})")
        if dev.get("max_input_channels", 0) > 0 and requested_lower in dev.get("name", "").lower():
            return idx

    log(f"[AUDIO] Input device name not found: {requested}")
    return None
...
if INPUT_DEVICE_NAME is not None:
    try:
        ...
        log(f"[AUDIO] Using input device: {device_info.get('name', INPUT_DEVICE_NAME)}")
    except Exception:
        log(f"[AUDIO] Using input device index: {INPUT_DEVICE_NAME}")
...
class BotGUI:
    ...
    def __init__(self, master):
        ...
        try:
            ...
            log(f"✅ Wake word loaded — model: {WAKE_WORD_MODEL}")
        except Exception as e:
            log(f"❌ Failed to load wake word model: {e}")
    ...
    def execute_action_and_get_result(self, action_data):
        ...
        log(f"⚡ ACTION: {raw_action} -> {action}")
            ...
            try:
                ...
                with DDGS() as ddgs:
                    ...
                    try:
                        ...
                        if results: 
                            log(f"[DEBUG] Found News: {results[0].get('title')}")
                    except Exception as e: 
                        log(f"[DEBUG] News Search Error: {e}")
                    
                    # 2. Text fallback
                    if not results:
                        log("[DEBUG] No news found, trying text search...")
                        try: 
                            results = list(ddgs.text(value, region='us-en', max_results=1))
                            if results: 
                                log(f"[DEBUG] Found Text: {results[0].get('title')}")
                        except Exception as e:
                            log(f"[DEBUG] Text Search Error: {e}")
                    ...
                    if results:
                        ...
                        log("[DEBUG] Search returned 0 results.")
                        return "SEARCH_EMPTY"
            except Exception as e:
                log(f"[DEBUG] Connection/Library Error: {e}")
                return "SEARCH_ERROR"
    ...
    def detect_wake_word_or_ptt(self):
        ...
        try:
            ...
            log(f"[AUDIO] Stream failed with defaults: {e}. Retrying with loose settings...")
            try:
                ...
                log(f"[CRITICAL] Wake Word Stream Error: {e2}")
                self.ptt_event.wait()
                return "PTT"
    ...
    def _listen_loop(self, stream_args, input_chunk_size, target_chunk_size, use_resampling):
        ...
        # Let's try to be less aggressive with reads.
         ...
         with sd.InputStream(**stream_args) as stream:
                log(f"[AUDIO] Listening with rate {stream_args['samplerate']} and block {stream_args['blocksize']}")
    ...
    def record_voice_adaptive(self, filename="input.wav"):
        ...
        try:
            ...
            log(f"❌ Adaptive recording failed: {e}")
            return None 
    ...
    def record_voice_ptt(self, filename="input.wav"):
        log("🎙️ Recording started (PTT mode)")
        time.sleep(0.5)
        samplerate = choose_input_samplerate(INPUT_DEVICE_NAME, CURRENT_CONFIG.get("input_sample_rate"))
        ...
        try:
            ...
            if cam_device in ("auto", "rpicam"):
                try:
                    ...
                    log("[CAM] Captured with rpicam-still")
                    return self._rotate_and_return()
                except (FileNotFoundError, subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
                    if cam_device == "rpicam":
                        log(f"[CAM] rpicam failed: {e}")
                        return None
                    log("[CAM] rpicam not available, trying USB webcam...")
            ...
            try:
                ...
                log(f"[CAM] Captured with ffmpeg from {dev}")
                return self._rotate_and_return()
            except (FileNotFoundError, subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
                log(f"[CAM] ffmpeg failed on {dev}: {e}")
                return None

        except Exception as e:
            log(f"[CAM] Unexpected error: {e}")
            return None
